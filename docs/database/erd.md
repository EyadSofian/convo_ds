...
## Campaign report export jobs (migration 0025)

```
campaign_report_exports      (id, tenant_id, requested_by_membership_id,
                              campaign_id NULL, format, state, attempt,
                              lease_owner, lease_expires_at, row_count,
                              content_text, content_sha256, error_code,
                              requested_at, started_at, completed_at, expires_at)
campaign_report_export_queue (export_id PK, tenant_id, available_at, created_at)
```

The export row is FORCE-RLS and keeps the filter and generated bytes inside the tenant boundary. The contentless queue exposes only `(tenant_id, export_id)` so `worker-report` can discover work before entering `withTenant`. A lease may be recovered twice; a third abandoned attempt closes as `export_attempts_exhausted`. Completed content carries a SHA-256 digest and expires after 24 hours. The API additionally binds status and download to the membership that requested the job.
