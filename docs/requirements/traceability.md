...
| IAM-14 | Custom roles grant only delegable permissions the actor holds | P1 | Escalation attempt rejected | partial | passed | n/a | n/a | `packages/domain/src/iam/delegation.ts` answers the two questions separately: `canAssignRole` (subset only — the delegable flag must not apply, or an Admin could never invite another Admin) and `canAuthorRole` (subset **and** delegable). 19 unit tests, including every non-delegable key refused even to an Owner. `canAssignRole`/`canGrantScopes` are enforced on the invitation path today. `partial` until custom-role **creation** exists as a write path. |
