...
| IAM-11 | `conversation.unassigned.preview` returns projected queue card only | P2 | Response schema has no snippet/timeline/notes/media/contact PII | implemented | passed | n/a | n/a | The queue card is built field by field from `QUEUE_CARD_FIELDS` on the server — a *projection*, never a conversation with fields hidden in the browser. Enforced on **both** surfaces from the same function: `GET T/conversations/unassigned` and every socket frame for a caller who may only preview. The unit test asserts the card's key set equals the permitted list exactly; the integration test serialises the frames and asserts the transcript, the attachment name, the contact name and the raw phone number appear nowhere in them |
...
| UX-09 | Every UI action calls a real endpoint; no setTimeout backend, localStorage DB or premature success toast | P1→P8 | Static rule + e2e assertion | partial | passed | n/a | n/a | **People** and **Channels** are wired end to end: every control calls a documented endpoint through `apps/web/src/api/`, the busy state is set before the request and the toast only after the response commits (`live.test.ts` holds a response open and asserts no toast exists while it is in flight). Both demo screens were deleted along with the `channel` demo action, which used to answer four verbs with a warning toast. Inbox, Broadcasts, Analytics and Settings are still backed by `data.ts` and make no request, so this stays `partial` |
