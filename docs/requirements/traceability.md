...
| CH-WA-05 | 24-hour customer-service window enforcement with template requirement outside it | P2 | Window closed → typed `WINDOW_EXPIRED`, draft preserved | partial | passed | n/a | n/a | `permitSend` decides against the connection's **frozen** matrix, refuses with `template_required` outside the window, `window_expired` where the channel has no template path, and `customer_initiation_required` for a cold Instagram or Messenger DM — with the boundary asserted to the millisecond. It is not yet wired to a send, because there is no send path, so "draft preserved" is untested |
...
| DEL-07 | Domain change + outbox event in one transaction | P2 | Crash between them impossible by construction | partial | passed | n/a | n/a | Holds for the **inbound** half: the queue row is inserted in the same transaction as the event it points at, so there is no window in which an event exists with nothing scheduled to process it. The outbound outbox does not exist yet |
