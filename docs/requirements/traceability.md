...
| DEL-18 | Per-conversation ordering via serialized dispatch gate with fencing + durable versions | P2 | Stale worker cannot publish authoritative state | partial | passed | n/a | n/a | Three things enforce one message per conversation on the wire: `DISTINCT ON` within a batch, `NOT EXISTS` against committed leases, and `pg_try_advisory_xact_lock` for the gap between them, with a partial unique index as the durable last line. Two messages for one recipient are claimed one sweep at a time — asserted. `dispatch_version` is incremented on every transition but is not yet *compared* on write, so the fencing half is recorded rather than enforced |
...
| ID | Requirement | Phase | Acceptance | Impl | Test | Live | Dep | Evidence |
|---|---|---|---|---|---|---|---|---|
| DEP-01 | Separately scalable process roles: api, ingress, realtime, workers (inbound/interactive/campaign/integration), ai | P1 | Each role has own concurrency/queue/resource config | partial | passed | n/a | n/a | `CONVO_PROCESS_ROLE` now accepts all seven roles and `CONVO_WORKER_CONCURRENCY` bounds a worker's parallelism, with an unknown role still refused before boot. The inbound worker exists as a service with a durable lease. Separate **processes** and their queue configuration are not built: today one process serves every role's routes |
