...
| Conversations assigned in period | `conversation_audit` acts `claim`,`assign`,`handoff` | Assignment event | `to_value` membership UUID | `conversation_audit.at` in range | Every target assignment counts; no deduplication of reassignments |
