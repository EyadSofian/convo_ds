...
export class AutomationService {
  ...
  runs(session: AuthenticatedSession, tenantId: string, query: AutomationRunsQuery): Promise<AutomationRunsPage> { return this.authorization.authorized(session,tenantId,'automation.read',async({sql}) => {
    ...
    const page = rows.rows.slice(0, query.limit); const last = page.at(-1); return { items: page.map(({ cursor_value: _cursor, ...run }) => run), nextCursor: rows.rows.length > query.limit && last !== undefined ? codec.encode(binding, { value: last.cursor_value, id: last.id }, 900) : null };
  }); }
...
function pageBinding(tenantId: string, query: unknown, sort: string) { const { cursor: _cursor, ...stable } = query as { readonly cursor?: string | null }; return { tenantId, filterHash: createHash('sha256').update(JSON.stringify(stable)).digest('hex'), sort }; }
