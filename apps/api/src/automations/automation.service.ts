...
/** The step types the executor can run today; any other is refused at activation rather than failing on every recipient. */
const EXECUTABLE_STEPS:ReadonlySet<string>=new Set(['condition','delay','send_whatsapp_template','add_label','remove_label','update_customer_field']);
function uuid(value:unknown):value is string{return typeof value==='string'&&UUID_PATTERN.test(value);}
function incomplete(message:string):ApiHttpError{return new ApiHttpError(409,'automation_step_incomplete',message);}
/**
 * Everything an active automation needs before its first run: each step is one
 * the executor supports and is fully configured, the resources it names exist
 * and are active, and a label target names its label. Checked here so an
 * operator fixes a draft instead of reading a failure on every recipient.
 */
async function validateActivation(sql:SqlExecutor,workflow:AutomationWorkflow):Promise<void>{
  ...
  for(const step of workflow.steps){
    ...
    else if(step.type==='add_label'||step.type==='remove_label'){const labelId=step.config['labelId'];if(!uuid(labelId)||(await sql.query(`SELECT 1 FROM labels WHERE id=$1 AND state='active'`,[labelId])).rows.length===0)throw incomplete('Choose an active label for the label step.');}
    else if(step.type==='update_customer_field'){const fieldId=step.config['fieldId'];if(!uuid(fieldId)||step.config['value']===undefined||(await sql.query(`SELECT 1 FROM custom_fields WHERE id=$1 AND target='contact' AND state='active'`,[fieldId])).rows.length===0)throw incomplete('Choose an active contact field and the value to set.');}
