...
describe('PeopleApi public credential flows', () => {
  it('maps recovery and invitation operations without leaking tokens into bodies unnecessarily', async () => {
    ...
    expect(JSON.parse(String(calls[2]?.init.body))).toEqual({ password: 'new password' });
  });
});
