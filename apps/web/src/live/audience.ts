import type { ConditionDocument, ConditionPredicate } from '@convo/domain';
 ...
 * campaign cannot apply exactly — a nested group, an "any" match, or a field
 * and operator this editor never writes. Refusing is the honest answer:
 * approximating a saved audience would send to people it did not name.
 */
...
export function editorAudience(state: AppState): EditorAudience {
  ...
  const firstHealthy = rowsOf(live.connections).find((connection) => connection.status === 'healthy')?.id ?? '';
  return {
    draft,
    filter: filterOf(draft, rowsOf(live.audiences)),
    connectionId: state.dialogForm['campaignConnection'] ?? campaign?.connection_id ?? firstHealthy,
  };
}
